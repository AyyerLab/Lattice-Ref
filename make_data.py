...
class DataGenerator:
    def __init__(self, N, num_samples, 
                       shape, scale, shifts, seed, 
                       add_noise=False, noise_mu=None, noise_k=None, 
                       apply_orientation=False, angles=None):
        self.N = N
        ...
        if self.APPLY_ORIENTATION:
            self.ANGLES = angles
            self.rotate_vals = np.random.uniform(self.ANGLES[0], self.ANGLES[1], size=self.NUM_SAMPLES)
    ...
    def generate_dataset(self):
        ...
        noise_vals = None
        rotated_ftobjs = np.zeros((self.NUM_SAMPLES, self.N, self.N), dtype=np.complex128)
    ...
    def save_data(self, filename):
        ...
        with h5py.File(filename, 'w') as f:
            ...
            if self.APPLY_ORIENTATION:
                f.create_dataset('angles', data=self.rotate_vals)
...
if __name__ == "__main__":
    ...
    (N, NUM_SAMPLES, SHAPE, SCALE, SHIFTS, 
     SEED, DATA_FILE, ADD_NOISE, NOISE_MU, NOISE_K, 
     APPLY_ORIENTATION, ANGLES) = makedata_config(config_file)
    generator = DataGenerator(N, NUM_SAMPLES, SHAPE, SCALE, SHIFTS, 
                              SEED, ADD_NOISE, NOISE_MU, NOISE_K, 
                              APPLY_ORIENTATION, ANGLES)
